...
class PlayState extends MusicBeatState
{
	...
	var camSettings:Map<String, Dynamic> = new Map();
	var camSettingsDAD:Map<String, Dynamic> = new Map();
	var camFacingDad:Bool = true;
	...
	override public function create()
	{
		...
		//setup cam settings
		camSettings.set('speed', 12);
		camSettings.set('ease', FlxEase.elasticInOut);
		camSettings.set('bumpMult', 1);
		camSettings.set('angleMult', 1);
		camSettings.set('dirMult', 1);
		camSettings.set('zoom', 0.9);
		camSettings.set('zSpeed', 5);
		camSettings.set('zEase', FlxEase.cubeOut);
		camSettings.set('zLimit', 1.35);

		camSettingsDAD.set('speed', 14);
		camSettingsDAD.set('ease', FlxEase.quartIn);
		camSettingsDAD.set('bumpMult', 2);
		camSettingsDAD.set('angleMult', 4);
		camSettingsDAD.set('dirMult', 4);
		camSettingsDAD.set('zoom', 1.1);
		camSettingsDAD.set('zSpeed', 2);
		camSettingsDAD.set('zEase', FlxEase.elasticOut);
		camSettingsDAD.set('zLimit', 3);
		...
		timeBar = new Bar(870, 10, 'timeBar', function() return songPercent, 0, 1);
		timeBar.scrollFactor.set();
		timeBar.alpha = 0;
		timeBar.visible = showTime;
		if(ClientPrefs.data.downScroll) timeBar.y = FlxG.height - timeBar.height - 10;
		uiGroup.add(timeBar);
		uiGroup.add(timeTxt);
		...
		scoreTxt.setFormat(Paths.font(getFontName()), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		smallScoreTxt = new FlxText(1091, 675, FlxG.width, "Score: 0", 20);
		smallScoreTxt.setFormat(Paths.font("getFontName()"), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		smallScoreTxt.scrollFactor.set();
		smallScoreTxt.borderSize = 1.25;
		if(ClientPrefs.data.downScroll) smallScoreTxt.y = FlxG.height - 675;
		uiGroup.add(smallScoreTxt);

		missText = new FlxText(1091, 650, FlxG.width, "Misses: 0", 20);
		missText.setFormat(Paths.font("getFontName()"), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		missText.scrollFactor.set();
		missText.borderSize = 1.25;
		if(ClientPrefs.data.downScroll) missText.y = FlxG.height - 650;
		uiGroup.add(missText);
	...
	function getFontName(){
		trace(SONG.song.toLowerCase() == "clownin-around");
		switch(SONG.song.toLowerCase()) {
			case "clownin-around": return "MS_PAIN.ttf";
			case "OGSB": return "consolas.ttf";
			default: return "AnisFont-Regular.ttf";
		}
	}
	...
	function grabCamSetting(settingToGrab:String = 'speed'):Dynamic {
		if(camFacingDad)
			return camSettingsDAD.get(settingToGrab);
		else
			return camSettings.get(settingToGrab);
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && gameOverTimer == null)
		{
			...
			if(ret != LuaUtils.Function_Stop)
			{
				...
				deathCounter++;
				FlxG.sound.play(Paths.sound('dashiDeath' + Std.string(FlxG.random.int(1, 5)) +  introSoundsSuffix), 0.6);
				...
				FlxTween.tween(camGame, {zoom: 0.9}, 0.5, {ease: grabCamSetting('zEase')});
				FlxTween.tween(camFollow, {x: (boyfriend.getMidpoint().x - 100) - boyfriend.cameraPosition[0] - boyfriendCameraOffset[0], y: (boyfriend.getMidpoint().y - 100) + boyfriend.cameraPosition[1] + boyfriendCameraOffset[1]}, 0.5, {ease: grabCamSetting('ease')});

				FlxG.camera.setFilters([]);
				
				FlxTween.tween(opponentVocals, {pitch: 0}, 0.5, {ease: FlxEase.quadOut});
				FlxTween.tween(vocals, {pitch: 0}, 0.5, {ease: FlxEase.quadOut});
				FlxTween.tween(FlxG.sound.music, {pitch: 0}, 0.5, {ease: FlxEase.quadOut, onComplete: function(twn:FlxTween) 
					{
						paused = true;
						canResync = false;
						persistentUpdate = false;
						...
						if(GameOverSubstate.deathDelay > 0)
						{
							gameOverTimer = new FlxTimer().start(GameOverSubstate.deathDelay, function(_)
							{
								...
								opponentVocals.stop();
								FlxG.sound.music.stop();
								openSubState(new GameOverSubstate(boyfriend, camGame.x, camGame.y, camGame.zoom)); //boyfriend
								gameOverTimer = null;
							});
						}
						else
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName) {
			...
			case 'Change Cam Prop':
				var prop = value1.split(',')[1];
				var val:Dynamic = flValue2;
				if(value2.startsWith('FlxEase.')) {
					if (Reflect.hasField(FlxEase, value2.split('FlxEase.')[1])) {
						val = Reflect.field(FlxEase, value2.split('FlxEase.')[1]);
					}else{
						addTextToDebug('Error setting camera ease: ' + value2, FlxColor.RED);
					}
				}
				if(value1.split(',')[0].toLowerCase() == "dad"){
					camSettingsDAD.set(prop, val);
				}else{
					camSettings.set(prop, val);
				}

			case 'Black Flash':
				if(flValue1 < 50) {
					//camGame.stopFlash();
					if(value2 == '') value2 = 'BLACK';
					camGame.flash(FlxColor.fromString(value2), flValue1);		
				}
	...
	public function moveCamera(isDad:Bool, force:Bool = false)
	{
		camFacingDad = isDad;
		FlxTween.cancelTweensOf(camFollow);
		FlxTween.cancelTweensOf(camGame.zoom);

		if(force || !boyfriend.stunned){
			if(isDad)
			{
				if(dad == null) return;
				...
				FlxTween.tween(camGame, {zoom: grabCamSetting('zoom')}, Conductor.stepCrochet * grabCamSetting('zSpeed') / 1000, {ease: grabCamSetting('zEase')});
				FlxTween.tween(camFollow, {x: (dad.getMidpoint().x + 150) + dad.cameraPosition[0] + opponentCameraOffset[0], y: (dad.getMidpoint().y - 100) + dad.cameraPosition[1] + opponentCameraOffset[1]}, (Conductor.stepCrochet * grabCamSetting('speed') / 1000), {ease: grabCamSetting('ease')});
			}
			else
			{
				...
				FlxTween.tween(camGame, {zoom: grabCamSetting('zoom')}, Conductor.stepCrochet * grabCamSetting('zSpeed') / 1000, {ease: grabCamSetting('zEase')});
				FlxTween.tween(camFollow, {x: (boyfriend.getMidpoint().x - 100) - boyfriend.cameraPosition[0] - boyfriendCameraOffset[0], y: (boyfriend.getMidpoint().y - 100) + boyfriend.cameraPosition[1] + boyfriendCameraOffset[1]}, (Conductor.stepCrochet * grabCamSetting('speed') / 1000), {ease: grabCamSetting('ease')});
			}
		}
	}
		...
		if(!note.hitCausesMiss) //Common notes
		{
			if(!note.noAnimation)
			{
				...
				/*if(!camFacingDad){
					...
					{
						case 0:
							FlxTween.tween(camGame, {x: -5 * grabCamSetting('dirMult')}, Conductor.stepCrochet * grabCamSetting('speed') / 1000, {ease: grabCamSetting('ease')});
						case 1:
							FlxTween.tween(camGame, {y: 5 * grabCamSetting('dirMult')}, Conductor.stepCrochet * grabCamSetting('speed') / 1000, {ease: grabCamSetting('ease')});
						case 2:
							FlxTween.tween(camGame, {y: -5 * grabCamSetting('dirMult')}, Conductor.stepCrochet * grabCamSetting('speed') / 1000, {ease: grabCamSetting('ease')});
						case 3:
							FlxTween.tween(camGame, {x: 5 * grabCamSetting('dirMult')}, Conductor.stepCrochet * grabCamSetting('speed') / 1000, {ease: grabCamSetting('ease')});
	...
	override function sectionHit()
	{
		if (SONG.notes[curSection] != null)
		{
			...
			if (camZooming && FlxG.camera.zoom < grabCamSetting('zLimit') && ClientPrefs.data.camZooms)
			{
				FlxG.camera.zoom += 0.015 * grabCamSetting('bumpMult');
				camHUD.zoom += 0.03 * 1;
			}
	...
	public function RecalculateRating(badHit:Bool = false) {
		...
		missText.text = 'Misses: ' + Std.string(songMisses);
		smallScoreTxt.text = 'Score: ' + Std.string(songScore);
	}
